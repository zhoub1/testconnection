...
class CdkDatazoneStack(Stack):
    ...
    This stack:
      - Creates a DataZone Domain, Project, and Environment
      - Sets up S3 buckets
      - Creates a Glue Crawler and Lake Formation permissions
      - Uses a custom resource to wait for the public DB to exist
    """
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        def validate_parameters():
            ...
            if not re.match(r'^[A-Za-z0-9]+$', domain_name):
                errors.append(f"Invalid domain name '{domain_name}'. Must be alphanumeric with no spaces.")
            if not re.match(r'^[a-z0-9]+$', project_name):
                errors.append(f"Invalid project name '{project_name}'. Must be lowercase alphanumeric, no spaces.")
            ...
            if not re.match(r'^[A-Za-z0-9_-]+$', environment_profile_name):
                errors.append(f"Invalid environment profile name '{environment_profile_name}'. Allowed: letters, numbers, underscores, hyphens.")
            cron_pattern = r'^cron\(.+\)$'
        ...
        def sanitize_bucket_name(name):
            ...
            if not (3 <= len(name) <= 63):
                raise ValueError(f"S3 bucket name '{name}' must be 3-63 chars long.")
            return name
        ...
        # 3. Project owner identifier
        project_owner_identifier = self.node.try_get_context("project_owner_identifier")
        if not project_owner_identifier:
            raise ValueError("Context var 'project_owner_identifier' is required (IAM ARN or SSO identifier).")
        ...
        # 5. IAM Role for a Lambda that triggers the Glue crawler
        ...
        # 8. Lake Formation Data Lake Admin
        ...
        # 11. DataZone Project Membership
        ...
        # 12. Environment Blueprint config
        ...
        # 16. Register S3 location in Lake Formation
        ...
        # 18. LakeFormation Permissions for the crawler on S3 location
        ...
        # 19. LakeFormation Permissions for the crawler on the pub DB
        ...
        # 21. Lake Formation perms for domain exec role on pub DB
        ...
        # 22. Lake Formation perms for domain exec role on sub DB
        ...
        #
        # 23. Wait for the *pub_db* to exist using a custom resource
        # This will poll Glue.getDatabase until the DB is found or 2 minutes pass.
        #
        ...
        # We only start waiting after the environment is created
        wait_for_pub_db.node.add_dependency(datazone_environment)

        #
        # 24. Make the pub DB resources wait for WaitForPubDB
        #
