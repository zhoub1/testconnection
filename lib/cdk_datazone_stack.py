...
# Import these for the custom waiting mechanism
...
class CdkDatazoneStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        * **data_source_schedule (String, Default: "cron(0 9 * * ? *)"):** Schedule for created data source ingestion.
        ...
        #
        # NEW: Add a custom resource that WAITs for the pub DB
        # ( environment_name_pub_db ) to exist before Lake Formation tries to reference it.
        #
        wait_for_pub_db = AwsCustomResource(
            ...
            on_create={
                ...
                "physicalResourceId": PhysicalResourceId.of(f"{environment_name}_pub_db-wait"),
                # We ignore "EntityNotFoundException" so it keeps retrying until it finds the DB or times out
                "ignoreErrorCodesMatching": "EntityNotFoundException"
            },
            # Re-check on update, too
            on_update={
                "service": "Glue",
                "action": "getDatabase",
                "parameters": {
            ...
            # Wait up to 2 minutes total for the DB to appear
            timeout=Duration.minutes(2)
        )
        # Make sure the environment is created before we start polling for the DB
        wait_for_pub_db.node.add_dependency(datazone_environment)

        # Data Source configuration depends on environment
        ...
        # LakeFormationPermissions referencing the pub DB must wait for the DB
        ...
        # Wait for environment AND the pub DB to exist
        ...
        #
        # The sub DB references remain as-is. We do NOT wait for environment_name_sub_db.
        # If the sub DB is never created before LakeFormation tries to grant perms, it may fail.
        #
